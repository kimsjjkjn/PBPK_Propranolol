...
- `STOPTIME = 120` ; Simulation end time (min): 10-min infusion + 2880-min post-infusion sampling
...
- `VT & d/dt(C_T) = Q_T * C_ab - Q_T * Cv_T`
  ...
  - There is no differential equation for Cv_T as blood outflow is different depending on the tissues. Therefore, the following conversion is used to define Cv_T:
    ...
    - Thus `Cv_T = (C_T / Kp) * RB`
  - Thus, the mass differential equation can be written again as `VT * d/dt(C_T) = Q_T * C_ab - C_T / Kp * RB`
- **Structure**:
  ...
  - Basic structure of mass in equation `(Xin_T): Xin_T = Q_T * C_T`
    ...
    - The following tissues had slight variations in mass in equations:
      ...
      - **Vein**: `Xin_ve = Xout_a + Xout_bo + Xout_b + Xout_h + Xout_k + Xout_li + Xout_m + Xout_sk`
        - Because vein compartment is where blood that outflow from each tissue is assembled, Xin_ve (venous inflow) is equivalent to the sum of Xout of other tissues except for artery, lung, gut, and spleen. This is because:
          ...
          -  **Gut & Spleen**: They drain via portal vein to the liver first: `Xin_li = (Q_li - Q_sp - Q_g) * C_ar + Xout_g + Xout_sp`. Their contribution reaches systemic vein via liver outflow Xout_li, so incluuding Xout_li only is sufficient.
      ...
      - **Liver**: `Xin_li = (Q_li - Q_sp - Q_g) * C_ar + (Xout_g) + (Xout_sp)`
        -  The liver has dual inflow - 1. hepatic artery carrying arterial blood, and 2. portal vein, which is exactly the outflow from gut and spleen. Thus writing blood flow as `Q_li − Q_sp − Q_g` avoids double-counting when Q_li is total hepatic flow.
...
## Blood flow
- Blood flow of each tissue is determined by multiplying cardiac output and spefific fraction (specific fracion can be found in literature or SimCYP).
  - e.g.
    - `Q = 80 ; cardiac output`
    - `Q_lg = Q ; lung`
